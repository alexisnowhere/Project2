...
function Feather(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(1.0, 1.0);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
Feather.prototype.type = "feather";

function Nest(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(1.95, 1.4);
}
Nest.prototype.type = "nest";
...
var stoneSpeed = 3, stoneDistance = 3;
Stone.prototype.act = function(step) {
  this.move += step * stoneSpeed;
  var stonePos = Math.sin(this.move) * stoneDistance;
  this.pos = this.basePos.plus(new Vector(stonePos, 0));
};
...
Feather.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};

Nest.prototype.act = function(step){
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    function speedBoost(){ playerXSpeed += 3; }
    speedBoost();
  }
  else if (type == "stone" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
    playerXSpeed = 7;
  }
